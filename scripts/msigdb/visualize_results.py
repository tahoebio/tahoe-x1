...
def compute_auprc_single_rep(results_dir, sigs):
    ...
    pred_files = [f for f in os.listdir(results_dir) if f.endswith("_benchmark_predictions.csv")]
    ...
    for fn in pred_files:
        ...
        preds_melted = preds.melt(ignore_index=False, var_name="sig", value_name="prediction").reset_index(names="gene")
        
        # Merge with signatures (add labels)
        with_sigs = preds_melted.merge(sigs.assign(observed=1), on=["gene", "sig"], how="left").fillna(0)
        
        # Calculate AUPRC
        auprc_scores[emb_name] = average_precision_score(with_sigs["observed"], with_sigs["prediction"])
...
def plot_multi_rep(results_dir, sigs, title="AUPRC Across Replicates"):
    ...
    ax = sns.barplot(data=combined, x='emb', y='AUPRC', errorbar="sd", estimator=np.mean, palette=palette)
