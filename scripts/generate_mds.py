...
def convert_to_mds(args: Tuple[str, str]) -> None:
    ...
    with MDSWriter(out=sub_out_root, columns=COLUMNS, compression=COMPRESSION, hashes=HASHES) as out:
        for sample in dataset:
            sample["id"] = torch.tensor(sample["id"], dtype=torch.int32)
            out.write(sample)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process datasets and write MDS files.")
    ...
    parser.add_argument("--train_suffix", default="/scgpt_old_dataset_train.dataset", help="Suffix for the train dataset.")
    parser.add_argument("--val_suffix", default="/scgpt_old_dataset_valid.dataset", help="Suffix for the validation dataset.")
    """
    ...
    TRAIN_SUFFIX = "/scgpt_old_dataset_train.dataset"
    VALID_SUFFIX = "/scgpt_old_dataset_valid.dataset"
    """
