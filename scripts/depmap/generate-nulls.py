...
Create null model predictions for both the marginal and cell line
specific essentiality tasks.
...
def null_marginal_task(base_path):
    obs = sc.read_h5ad(os.path.join(base_path, "gene-embs/loading15-lt5gt70-bin.h5ad")).obs
    ...
    for fold in range(5):
        ...
        np.save(os.path.join(base_path, f"gene-embs/results/null-lt5gt70-bin-fold-{fold}-val-true.npy"), val_labels)
        np.save(os.path.join(base_path, f"gene-embs/results/null-lt5gt70-bin-fold-{fold}-val-proba.npy"), val_probas)
        np.save(os.path.join(base_path, f"gene-embs/results/null-lt5gt70-bin-fold-{fold}-val-pred.npy"), val_preds)
...
def null_cell_line_specific_task(base_path, null_dict, subset):
    obs = sc.read_h5ad(os.path.join(base_path, f"gene-embs/gene_idx+pca15-{subset}.h5ad")).obs
    ...
    for fold in range(5):
        ...
        np.save(os.path.join(base_path, f"gene-embs/results/null-{subset}-fold-{fold}-val-true.npy"), val_labels)
        np.save(os.path.join(base_path, f"gene-embs/results/null-{subset}-fold-{fold}-val-pred.npy"), val_preds)
...
def main(base_path):
    ...
    log.info(f"saving dicitonary of null predictions for cell line specific task to {outpath}")
    ...
    subsets = ["0to10", "10to20", "20to30", "30to40", "40to50", "50to60", "60to70", "70to80", "80to90", "90to100"]
    for s in subsets:
